...
function Dashboard() {
  const { stats, deviceHistory, otpSummary, loading, error, refetch } = useDashboard();
  ...
  const blockedDevices = stats?.blocked_devices ?? 0;
  const inactiveDevices = stats?.inactive_devices ?? 0;
  ...
  const historyColumns = [
    ...
    },
    {
      ...
      render: (row) =>
        row.status ? (
          <Badge variant={row.status === 'success' ? 'success' : 'default'}>{row.status}</Badge>
        ) : null,
    },
  ];
    ...
    <MainLayout title="Dashboard">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={refetch} className="gap-2 ml-4 shrink-0">
        ...
        <Card title="Device Status Overview" className="lg:col-span-2">
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
              <span className="font-medium text-gray-900">Total Registered Devices</span>
          ...
          {otpSummary ? (
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center pb-3 border-b">
                ...
                <Badge variant={error ? 'danger' : 'success'}>{error ? 'Error' : 'Healthy'}</Badge>
              </div>
              <div className="flex justify-between items-center">
