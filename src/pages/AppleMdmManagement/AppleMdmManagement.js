...
function AppleMdmManagement() {
  ...
  const [deviceForToggle, setDeviceForToggle] = useState(null);
  ...
  const handleViewCommands = useCallback(
    async (device) => {
      ...
      if (result.success) {
        ...
        setNotification({
          type: "error",
          message: result.error || "Failed to load commands"
        });
      }
  ...
  const closeToggleRemovableModal = () => {
    ...
    setDeviceForToggle(null);
  };

  const handleToggleMdmRemovable = async () => {
    if (!deviceForToggle) return;
    const deviceId = deviceForToggle.id || deviceForToggle.udid;
  ...
  const stats = {
    ...
    online: devices.filter(
      (d) =>
        ...
        (d.lastSeen
          ? new Date(d.lastSeen) > new Date(Date.now() - 5 * 60 * 1000)
          : false)
    ).length,
    ...
    pendingCommands: devices.reduce(
      (sum, d) => sum + (d.pendingCommands ?? 0),
      0
    )
  };
  ...
  const deviceColumns = [
    ...
    },
    {
      ...
      render: (row) => {
        const status =
          row.enrollmentStatus ?? row.enrollment_status ?? "unknown";
        const map =
          ENROLLMENT_STATUS_MAP[status?.toLowerCase()] ||
          ENROLLMENT_STATUS_MAP.pending;
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const map =
          CAMERA_STATUS_MAP[status?.toLowerCase()] || CAMERA_STATUS_MAP.unknown;
        return <Badge variant={map.variant}>{map.label}</Badge>;
      }
    },
    {
    ...
    },
    {
      ...
      render: (row) => {
        const isCameraDisabled =
          (row.cameraStatus ?? row.camera_status ?? "").toLowerCase() ===
          "disabled";
        return (
  ...
  const commandColumns = [
    {
      key: "commandUuid",
      label: "Command UUID",
      render: (row) => row.commandUuid ?? row.command_uuid ?? "—"
    },
    {
      key: "commandType",
      label: "Type",
      render: (row) => row.commandType ?? row.command_type ?? "—"
    },
    {
      key: "queuedAt",
      label: "Queued",
      render: (row) =>
        (row.queuedAt ?? row.queued_at)
          ? formatDateTime(row.queuedAt ?? row.queued_at)
          : "—"
    },
    {
      key: "acknowledgedAt",
      label: "Acknowledged",
      render: (row) =>
        (row.acknowledgedAt ?? row.acknowledged_at)
          ? formatDateTime(row.acknowledgedAt ?? row.acknowledged_at)
          : "—"
    },
    {
      key: "completedAt",
      ...
      render: (row) =>
        (row.completedAt ?? row.completed_at)
          ? formatDateTime(row.completedAt ?? row.completed_at)
          : "—"
    },
    {
      key: "status",
      ...
      render: (row) => {
        ...
        const map = COMMAND_STATUS_MAP[s?.toLowerCase()] || {
          label: s,
          variant: "secondary"
        };
        return <Badge variant={map.variant}>{map.label}</Badge>;
    ...
    },
    {
      ...
      render: (row) =>
        (row.timestamp ?? row.createdAt ?? row.created_at)
          ? formatDateTime(row.timestamp ?? row.createdAt ?? row.created_at)
          : "—"
    }
  ];
        ...
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-gray-700">
              ...
              {selectedDevice?.deviceName ?? selectedDevice?.udid ?? "—"}
            </p>
            <p className="text-sm text-gray-700">
          ...
          {confirmAction === "disable" && (
            <>
              <p className="text-gray-700">
                Are you sure you want to <strong>disable the camera</strong> on
                this device?
              </p>
              <p className="text-sm text-amber-700 bg-amber-50 p-3 rounded-lg">
                The camera will be disabled via MDM command. The device must be
                online to receive the command.
              </p>
            </>
          )}
          ...
          {confirmAction === "enable" && (
            <>
              <p className="text-gray-700">
                Are you sure you want to <strong>enable the camera</strong> on
                this device?
              </p>
              <p className="text-sm text-green-700 bg-green-50 p-3 rounded-lg">
                The camera will be enabled via MDM command. The device must be
                online to receive the command.
              </p>
            </>
          )}
          ...
          {confirmAction === "refresh" && (
            <>
              <p className="text-gray-700">
                This will <strong>refresh the device status</strong> from the
                MDM server.
              </p>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-900 mt-1">
                      {(selectedDevice.enrollmentDate ??
                      selectedDevice.enrollment_date)
                        ? formatDateTime(
                            selectedDevice.enrollmentDate ??
                              selectedDevice.enrollment_date
                          )
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-900 mt-1">
                      {(
                        selectedDevice.cameraStatus ??
                        selectedDevice.camera_status ??
                        "unknown"
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-900 mt-1">
                      {(() => {
                        const s =
                          selectedDevice.enrollmentStatus ??
                          selectedDevice.enrollment_status ??
                          "unknown";
                        const map = ENROLLMENT_STATUS_MAP[s?.toLowerCase()] || {
                          label: s,
                          variant: "secondary"
                        };
                        return <Badge variant={map.variant}>{map.label}</Badge>;
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-900 mt-1">
                    {selectedDevice.pendingCommands ??
                      selectedDevice.pending_commands ??
                      0}
                  </p>
                </div>
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-900 mt-1">
                    {selectedDevice.completedCommands ??
                      selectedDevice.completed_commands ??
                      0}
                  </p>
                </div>
              </div>
      ...
      >
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-gray-700">
              <strong>Device:</strong>{" "}
              {deviceForToggle?.deviceName ?? deviceForToggle?.udid ?? "—"}
            </p>
            <p className="text-sm text-gray-700">
              <strong>Serial:</strong>{" "}
              {deviceForToggle?.serialNumber ??
                deviceForToggle?.serial_number ??
                "—"}
            </p>
          </div>
