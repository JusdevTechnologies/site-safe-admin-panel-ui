...
export const mdmService = {
  ...
  getProfiles: async (params = {}) => {
    const response = await apiClient.get(API_ENDPOINTS.MDM_PROFILES, {
      params
    });
    return {
  ...
  installProfile: async (udid, profilePayload) => {
    const response = await apiClient.post(API_ENDPOINTS.MDM_INSTALL_PROFILE, {
      udid,
      profilePayload
    });
  ...
  removeProfile: async (udid, profileIdentifier) => {
    const response = await apiClient.post(API_ENDPOINTS.MDM_REMOVE_PROFILE, {
      udid,
      profileIdentifier
    });
    return extractData(response);
  ...
  toggleMdmRemovable: async (deviceId, removable) => {
    const response = await apiClient.post(
      API_ENDPOINTS.MDM_TOGGLE_REMOVABLE(deviceId),
      { removable }
    );
    return extractData(response);
  },
  ...
  getCommands: async (params = {}) => {
    const response = await apiClient.get(API_ENDPOINTS.MDM_COMMANDS, {
      params
    });
